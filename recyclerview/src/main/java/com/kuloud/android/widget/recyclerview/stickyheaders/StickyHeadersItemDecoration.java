...
public class StickyHeadersItemDecoration extends RecyclerView.ItemDecoration {
                ...
                if ((i == 0 && headerStore.isSticky()) || headerStore.isHeader(holder)) {
                    ...
                    if (header.getVisibility() == View.VISIBLE) {
                        ...
                        if (headerStore.isHeader(holder) && i == 0) {
                            y = 0;
                        } else if (headerStore.isSticky() && lastY != null
                                && lastY < y + headerHeight) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent,
        ...
        boolean isHeader = lp.isItemRemoved() ? headerStore.wasHeader(holder) : headerStore
                .isHeader(holder);
    ...
    public void registerAdapterDataObserver(RecyclerView.Adapter<? extends ViewHolder> adapter) {
        adapter.registerAdapterDataObserver(adapterDataObserver);
    }

    private float getHeaderY(View item, RecyclerView.LayoutManager lm) {
        return headerStore.isSticky() && lm.getDecoratedTop(item) < 0 ? 0 : lm
                .getDecoratedTop(item);
    }
