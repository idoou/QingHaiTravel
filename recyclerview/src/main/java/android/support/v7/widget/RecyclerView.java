...
import com.kuloud.android.widget.recyclerview.R;
...
/**
 * A flexible view for providing a limited window into a large data set. <h3>
 * Glossary of terms:</h3>
 * <ul>
 * <li><em>Adapter:</em> A subclass of {@link Adapter} responsible for providing
 * views that represent items in a data set.</li>
 * <li><em>Position:</em> The position of a data item within an <em>Adapter</em>
 * .</li>
 ...
 * <li><em>Binding:</em> The process of preparing a child view to display data
 * corresponding to a <em>position</em> within the adapter.</li>
 * <li><em>Recycle (view):</em> A view previously used to display data for a
 * specific adapter position may be placed in a cache for later reuse to display
 * the same type of data again later. This can drastically improve performance
 * by skipping initial layout inflation or construction.</li>
 * <li><em>Scrap (view):</em> A child view that has entered into a temporarily
 * detached state during layout. Scrap views may be reused without becoming
 * fully detached from the parent RecyclerView, either unmodified if no
 * rebinding is required or modified by the adapter if the view was considered
 * <em>dirty</em>.</li>
 * <li><em>Dirty (view):</em> A child view that must be rebound by the adapter
...
public class RecyclerView extends ViewGroup {
    ...
    /**
     * On Kitkat, there is a bug which prevents DisplayList from being
     * invalidated if a View is two levels deep(wrt to ViewHolder.itemView).
     * DisplayList can be invalidated by setting View's visibility to INVISIBLE
     * when View is detached. On Kitkat, Recycler recursively traverses itemView
     * and invalidates display list for each ViewGroup that matches this
     * criteria.
     */
    ...
    /**
     * Prior to L, there is no way to query this variable which is why we
     * override the setter and track it here.
     */
        ...
        public void run() {
            ...
            if (mDataSetHasChangedAfterLayout) {
                ...
                if (!mLayoutRequestEaten) {
                    // We run this after pre-processing is complete so that
                    // ViewHolders have their
                    // final adapter positions. No need to run it if a layout is
                    // already requested.
    ...
    /**
     * Set to true when an adapter data set changed notification is received. In
     * that case, we cannot run any animations since we don't know what
     * happened.
     */
    private boolean mDataSetHasChangedAfterLayout = false;

    /**
     * This variable is set to true during a dispatchLayout and/or scroll. Some
     * methods should not be called during these periods (e.g. adapter data
     * change). Doing so will create hard to find bugs so we better check it and
     * throw an exception.
     * 
    ...
    /**
     * Returns the accessibility delegate compatibility implementation used by
     * the RecyclerView.
     * 
    ...
    /**
     * Sets the accessibility delegate compatibility implementation used by
     * RecyclerView.
     * 
     ...
     * {@link #setAdapter(Adapter)} but assumes existing adapter and the new
     * adapter uses the same {@link ViewHolder} and does not clear the
     * RecycledViewPool.
     * <p>
     ...
     *            recycle all existing Views. If adapters have stable ids and/or
     *            you want to animate the disappearing views, you may prefer to
     *            set this to false.
     ...
     * <p>
     * When adapter is changed, all existing views are recycled back to the
     * pool. If the pool has only one adapter, it will be cleared.
     * 
     * @param adapter The new adapter to set, or null to set no adapter.
     ...
     * @param compatibleWithPrevious If true, the new adapter is using the same
     *            View Holders and item types with the current adapter (helps us
     *            avoid cache invalidation).
     * @param removeAndRecycleViews If true, we'll remove and recycle all
     *            existing views. If compatibleWithPrevious is false, this
    ...
    private void setAdapterInternal(Adapter adapter, boolean compatibleWithPrevious,
        ...
        if (!compatibleWithPrevious || removeAndRecycleViews) {
            ...
            // recyclerView.children. This may not be true if item animator's
            // end does not work as
            // expected. (e.g. not release children instantly). It is safer to
            // use mLayout's child
            // count.
            if (mLayout != null) {
    ...
    /**
     * Register a listener that will be notified whenever a child view is
     * recycled.
     * <p>
     * This listener will be called when a LayoutManager or the RecyclerView
     * decides that a child view is no longer needed. If an application
     * associates expensive or heavyweight data with item views, this may be a
     * good place to release or free those resources.
     * </p>
     * 
    ...
    public void setLayoutManager(LayoutManager layout) {
        ...
        // TODO We should do this switch a dispachLayout pass and animate
     ...
     * several data sets with the same kinds of item views displayed by a
     ...
     * 
     * @param extension ViewCacheExtension to be used or null if you want to
     ...
     *            at. If this value is negative the decoration will be added at
     *            the end.
     */
     ...
     * implement scrolling logic, rather forwards the call to
     ...
     * {@link LayoutManager#smoothScrollToPosition(RecyclerView, State, int)}
     * and create a {@link SmoothScroller}.
     * <p>
     * {@link LayoutManager} is responsible for creating the actual scroll
     * action. If you want to provide a custom smooth scroll logic, override
     * {@link LayoutManager#smoothScrollToPosition(RecyclerView, State, int)} in
     * your LayoutManager.
     * 
            ...
            if (supportsChangeAnimations()) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (holder != null && holder.mShadowingHolder != null) {
                        ...
                        View shadowingView = shadowingHolder != null ? shadowingHolder.itemView
                                : null;
     ...
     * {@link RecyclerView.LayoutManager#computeHorizontalScrollOffset(RecyclerView.State)}
     ...
     * <p>
     * If you want to support scroll bars, override
     * {@link RecyclerView.LayoutManager#computeHorizontalScrollExtent(RecyclerView.State)}
     * in your LayoutManager.
     * </p>
     * 
     ...
     * <p>
     * If you want to support scroll bars, override
     * {@link RecyclerView.LayoutManager#computeHorizontalScrollRange(RecyclerView.State)}
     * in your LayoutManager.
     * </p>
     * 
     ...
     * vertical range. This value is used to compute the length of the thumb
     ...
     * <p>
     * If you want to support scroll bars, override
     * {@link RecyclerView.LayoutManager#computeVerticalScrollOffset(RecyclerView.State)}
     * in your LayoutManager.
     * </p>
     * 
    ...
    /**
     * <p>
     * Compute the vertical extent of the vertical scrollbar's thumb within the
     * vertical range. This value is used to compute the length of the thumb
     * within the scrollbar's track.
     * </p>
     * <p>
     ...
     * <p>
     * If you want to support scroll bars, override
     * {@link RecyclerView.LayoutManager#computeVerticalScrollExtent(RecyclerView.State)}
     * in your LayoutManager.
     * </p>
     * 
     ...
     * <p>
     * If you want to support scroll bars, override
     * {@link RecyclerView.LayoutManager#computeVerticalScrollRange(RecyclerView.State)}
     * in your LayoutManager.
     * </p>
     * 
    ...
    /**
     * Checks if RecyclerView is in the middle of a layout or scroll and throws
     * an {@link IllegalStateException} if it <b>is not</b>.
     * 
    ...
    /**
     * Checks if RecyclerView is in the middle of a layout or scroll and throws
     * an {@link IllegalStateException} if it <b>is</b>.
     * 
     * @param message The message for the exception. Can be null.
     * @see #assertInLayoutOrScroll(String)
     */
    ...
    /**
     * Add an {@link OnItemTouchListener} to intercept touch events before they
     * are dispatched to child views or this view's standard scrolling behavior.
     * <p>
     * Client code may use listeners to implement item manipulation behavior.
     * Once a listener returns true from
     * {@link OnItemTouchListener#onInterceptTouchEvent(RecyclerView, MotionEvent)}
     * its {@link OnItemTouchListener#onTouchEvent(RecyclerView, MotionEvent)}
     * method will be called for each incoming MotionEvent until the end of the
     * gesture.
     * </p>
     * 
    ...
    /**
     * Sets the {@link ItemAnimator} that will handle animations involving
     * changes to the items in this RecyclerView. By default, RecyclerView
     * instantiates and uses an instance of {@link DefaultItemAnimator}. Whether
     * item animations are enabled for the RecyclerView depends on the
     * ItemAnimator and whether the LayoutManager
     * {@link LayoutManager#supportsPredictiveItemAnimations() supports item
     * animations}.
     * 
     * @param animator The ItemAnimator being set. If null, no animations will
     *            occur when changes occur to the items in this RecyclerView.
     */
     ...
     * indicates that there is no animator and that item changes will happen
     * without any animations. By default, RecyclerView instantiates and uses an
     * instance of {@link DefaultItemAnimator}.
     * 
     * @return ItemAnimator The current ItemAnimator. If null, no animations
     *         will occur when changes occur to the items in this RecyclerView.
     */
    ...
    /**
     * Consumes adapter updates and calculates which type of animations we want
     * to run. Called in onMeasure and dispatchLayout.
     * <p>
     * This method may process only the pre-layout state of updates or all of
     * them.
     */
    private void processAdapterUpdatesAndSetAnimationFlags() {
        if (mDataSetHasChangedAfterLayout) {
            // Processing these items have no value since data set changed
            // unexpectedly.
        ...
        // cause a
        // pre-layout step)
        // If layout supports predictive animations, pre-process to decide if we
        // want to run them
    ...
    /**
     * Wrapper around layoutChildren() that handles animating changes caused by
     * layout. Animations work on the assumption that there are five different
     * kinds of items in play: PERSISTENT: items are visible before and after
     * layout REMOVED: items were visible before layout and were removed by the
     * app ADDED: items did not exist before layout and were added by the app
     * DISAPPEARING: items exist in the data set before/after, but changed from
     * visible to non-visible in the process of layout (they were moved off
     * screen as a side-effect of other changes) APPEARING: items exist in the
     * data set before/after, but changed from non-visible to visible in the
     * process of layout (they were moved on screen as a side-effect of other
     * changes) The overall approach figures out what items exist before/after
     * layout and infers one of the five above states for each of the items.
     * Then the animations are set up accordingly: PERSISTENT views are moved (
     * {@link ItemAnimator#animateMove(ViewHolder, int, int, int, int)}) REMOVED
     * views are removed ({@link ItemAnimator#animateRemove(ViewHolder)}) ADDED
     * views are added ({@link ItemAnimator#animateAdd(ViewHolder)})
     * DISAPPEARING views are moved off screen APPEARING views are moved on
     * screen
     */
    void dispatchLayout() {
        if (mAdapter == null) {
        ...
        if (mState.mRunPredictiveAnimations) {
            ...
            // Save old positions so that LayoutManager can run its mapping
            // logic.
            saveOldPositions();
            // processAdapterUpdatesAndSetAnimationFlags already run pre-layout
            // animations.
            ...
            // we don't process disappearing list because they may re-appear in
            // post layout pass.
            clearOldPositions();
            mAdapterHelper.consumePostponedUpdates();
        } else {
            clearOldPositions();
            // in case pre layout did run but we decided not to run predictive
            // animations.
            mAdapterHelper.consumeUpdatesInOnePass();
            if (mState.mOldChangedHolders != null) {
                int count = mChildHelper.getChildCount();
        ...
        if (mState.mRunSimpleAnimations) {
            ...
            // traverse reverse in case view gets recycled while we are
            // traversing the list.
            for (int i = count - 1; i >= 0; i--) {
                ...
                // We probably don't need this check anymore since these views
                // are removed from
    ...
    /**
     * Returns a unique key to be used while handling change animations. It
     * might be child's position or stable id depending on the adapter type.
     */
    ...
    public void draw(Canvas c) {
        ...
        // TODO If padding is not 0 and chilChildrenToPadding is false, to draw
        // glows properly, we
        // need find children closest to edges. Not sure if it is worth the
        // effort.
        boolean needsInvalidate = false;
        ...
        // with them.
        // Invalidate RecyclerView to re-draw decorators. This is still
        // efficient because children's
    ...
    void viewRangeUpdate(int positionStart, int itemCount) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (holder.mPosition >= positionStart && holder.mPosition < positionEnd) {
                // We re-bind these view holders after pre-processing is
                // complete so that
            ...
            if (holder.isRemoved() || holder.isInvalid()) {
                ...
                if (holder.getItemViewType() == type) {
                    ...
                    if (!holder.isChanged() || !supportsChangeAnimations()) {
                        ...
                        // Don't rebind changed holders if change animations are
                        // enabled.
                        // We want the old contents for the animation and will
                        // get a new
                        // holder for the new contents.
                        requestLayout();
                    }
                } else {
                    // binding to a new view will need re-layout anyways. We can
                    // as well trigger
    ...
    /**
     * Invalidates all ItemDecorations. If RecyclerView has item decorations,
     * calling this method will trigger a {@link #requestLayout()} call.
     */
    ...
    ViewHolder findViewHolderForPosition(int position, boolean checkNewPosition) {
        ...
        // This method should not query cached views. It creates a problem
        // during adapter updates
    ...
    public ViewHolder findViewHolderForItemId(long id) {
        ...
        // this method should not query cached views. They are not children so
        // they
        ...
        public void run() {
            ...
            if (scroller.computeScrollOffset()) {
                ...
                if (scroller.isFinished() || !fullyConsumedScroll) {
                    setScrollState(SCROLL_STATE_IDLE); // setting state to idle
                                                       // will stop this.
                } else {
                    postOnAnimation();
                }
    ...
    private class RecyclerViewDataObserver extends AdapterDataObserver {
        ...
        public void onChanged() {
            ...
            if (mAdapter.hasStableIds()) {
                ...
                // This is more important to implement now since this callback
                // will disable all
     ...
     * RecycledViewPool and use
     * {@link RecyclerView#setRecycledViewPool(RecycledViewPool)}.
     * <p>
     * RecyclerView automatically creates a pool for itself if you don't provide
     * one.
     */
    public static class RecycledViewPool {
         ...
         * <p>
         * RecycledViewPool will clear its cache if it has only one adapter
         * attached and the new adapter uses a different ViewHolder than the
         * oldAdapter.
         * 
         * @param oldAdapter The previous adapter instance. Will be detached.
         * @param newAdapter The new adapter instance. Will be attached.
         * @param compatibleWithPrevious True if both oldAdapter and newAdapter
         *            are using the same ViewHolder and view types.
         */
     ...
     * {@link android.view.View#isLayoutRequested() requested layout} may be
    ...
    public final class Recycler {
        ...
        private final List<ViewHolder> mUnmodifiableAttachedScrap = Collections
                .unmodifiableList(mAttachedScrap);
        ...
        boolean validateViewHolderForOffsetPosition(ViewHolder holder) {
            ...
            if (holder.mPosition < 0 || holder.mPosition >= mAdapter.getItemCount()) {
                ...
                Log.e(TAG, "Inconsistency detected. Invalid view holder adapter position: " + holder.mPosition);
                return false;
            }
            if (!mState.isPreLayout()) {
        ...
        /**
         * Binds the given View to the position. The View can be a View
         * previously retrieved via {@link #getViewForPosition(int)} or created
         * by {@link Adapter#onCreateViewHolder(ViewGroup, int)}.
         * <p>
         * Generally, a LayoutManager should acquire its views via
         * {@link #getViewForPosition(int)} and let the RecyclerView handle
         * caching. This is a helper method for LayoutManager who wants to
         * handle its own recycling logic.
         * <p>
         * Note that, {@link #getViewForPosition(int)} already binds the View to
         * the position so you don't need to call this method unless you want to
         * bind this View to another position.
         * 
        ...
        /**
         * RecyclerView provides artificial position range (item count) in
         * pre-layout state and automatically maps these positions to
         * {@link Adapter} positions when {@link #getViewForPosition(int)} or
         * {@link #bindViewToPosition(View, int)} is called.
         * <p>
         * Usually, LayoutManager does not need to worry about this. However, in
         * some cases, your LayoutManager may need to call some custom component
         * with item positions in which case you need the actual adapter
         * position instead of the pre layout position. You can use this method
         * to convert a pre-layout position to adapter (post layout) position.
         * <p>
         * Note that if the provided position belongs to a deleted ViewHolder,
         * this method will return -1.
         * <p>
         * Calling this method in post-layout state returns the same value back.
         * 
         * @param position The pre-layout position to convert. Must be greater
         *            or equal to 0 and less than {@link State#getItemCount()}.
         */
        ...
        /**
         * Obtain a view initialized for the given position. This method should
         * be used by {@link LayoutManager} implementations to obtain views to
         * represent data from an {@link Adapter}.
         * <p>
         ...
         * <p>
         * A view must be fully detached before it may be recycled. If the View
         * is scrapped, it will be removed from scrap list.
         * </p>
         * 
        ...
        public void recycleView(View view) {
            // This public recycle method tries to make view recycle-able since
            // layout manager
            // intended to recycle this view (e.g. even if it is in scrap or
            // change cache)
            ViewHolder holder = getChildViewHolderInt(view);
            if (holder.isScrap()) {
                holder.unScrap();
        ...
        /**
         * Internally, use this method instead of
         * {@link #recycleView(android.view.View)} to catch potential bugs.
         * 
         * @param view
         */
        ...
        /**
         * Tries to recyle a cached view and removes the view from the list if
         * and only if it is recycled.
         * 
        ...
        /**
         * internal implementation checks if view is scrapped or attached and
         * throws an exception if so. Public version un-scraps before calling
         * recycle.
         */
        void recycleViewHolderInternal(ViewHolder holder) {
            ...
            // even if the holder is not removed, we still call this method so
            // that it is removed
         ...
         * done to update the recycler's internal bookkeeping.
         */
         ...
         * @param dryRun Does a dry run, finds the ViewHolder but does not
         *            remove
        ...
        ViewHolder getScrapViewForPosition(int position, int type, boolean dryRun) {
            ...
            if (!dryRun) {
                ...
                if (view != null) {
                    // ending the animation should cause it to get recycled
                    // before we reuse it
            ...
            for (int i = 0; i < cacheSize; i++) {
                ...
                // invalid view holders may be in cache if adapter has stable
                // ids as they can be
                ...
                if (holder.getItemId() == id && !holder.wasReturnedFromScrap()) {
                    if (type == holder.getItemViewType()) {
                        ...
                        if (holder.isRemoved()) {
                            ...
                            // rebind
                            // > item is removed then added to another position
                            // and we are in
                            // post layout.
                            // >> remove removed and invalid flags, add update
                            // flag to rebind
                            // because item was invisible to us and we don't
         ...
         * @param applyToPreLayout If true, changes will affect ViewHolder's
         *            pre-layout position, if false, they'll be applied before
            ...
            for (int i = cachedCount - 1; i >= 0; i--) {
                ...
                if (holder != null) {
                    if (holder.getPosition() >= removedEnd) {
                        ...
                        // cache.
                        if (!tryToRecycleCachedViewAt(i)) {
                            // if we cannot recycle it, at least invalidate so
                            // that we won't return
                            // it by position.
                            holder.addFlags(ViewHolder.FLAG_INVALID);
                            if (DEBUG) {
                                Log.d(TAG, "offsetPositionRecordsForRemove cached " + i +
        ...
        void viewRangeUpdate(int positionStart, int itemCount) {
            ...
            for (int i = 0; i < cachedCount; i++) {
                ...
                if (pos >= positionStart && pos < positionEnd) {
                    holder.addFlags(ViewHolder.FLAG_UPDATE);
                    // cached views should not be flagged as changed because
        ...
        void markKnownViewsInvalid() {
            if (mAdapter != null && mAdapter.hasStableIds()) {
                ...
                // we cannot re-use cached views in this case. Recycle the ones
                // we can and flag
                // the remaining as invalid so that they can be recycled later
                // on (when their
                // animations end.)
    ...
    /**
     * ViewCacheExtension is a helper class to provide an additional layer of
     * view caching that can ben controlled by the developer.
     * <p>
     * When {@link Recycler#getViewForPosition(int)} is called, Recycler checks
     * attached scrap and first level cache to find a matching View. If it
     * cannot find a suitable View, Recycler will call the
     * {@link #getViewForPositionAndType(Recycler, int, int)} before checking
     * {@link RecycledViewPool}.
     * <p>
     * Note that, Recycler never sends Views to this method to be cached. It is
     * developers responsibility to decide whether they want to keep their Views
     * in this custom cache or let the default recycling policy handle it.
     */
    public abstract static class ViewCacheExtension {
         ...
         * <p>
         * This method should <b>not</b> create a new View. Instead, it is
         * expected to return an already created View that can be re-used for
         * the given type and position. If the View is marked as ignored, it
         * should first call {@link LayoutManager#stopIgnoringView(View)} before
         * returning the View.
         * <p>
         * RecyclerView will re-bind the returned View to the position if
         * necessary.
         * 
         * @param recycler The Recycler that can be used to bind the View
         * @param position The adapter position
         * @param type The type of the View, defined by adapter
         * @return A View that is bound to the given position or NULL if there
         *         is no View to re-use
    ...
    public static abstract class Adapter<VH extends ViewHolder> {
         ...
         * {@link #onBindViewHolder(ViewHolder, int)}. Since it will be re-used
         * to display different items in the data set, it is a good idea to
         * cache references to sub views of the View to avoid unnecessary
         ...
         * position.
         * <p>
         * Note that unlike {@link android.widget.ListView}, RecyclerView will
         * not call this method again if the position of the item changes in the
         * data set unless the item itself is invalidated or the new position
         * cannot be determined. For this reason, you should only use the
         * <code>position</code> parameter while acquiring the related data item
         ...
         * {@link ViewHolder#getPosition()} which will have the updated
         * position.
         * 
        ...
        /**
         * This method calls {@link #onCreateViewHolder(ViewGroup, int)} to
         * create a new {@link ViewHolder} and initializes some private fields
        ...
        public final VH createViewHolder(ViewGroup parent, int viewType) {
            ...
            ItemClickSupport itemClick = (ItemClickSupport) parent
                    .getTag(R.id.twowayview_item_click_support);
            holder.itemView.setTag(R.id.twowayview_item_click_support, itemClick);
            return holder;
        }

        /**
         * This method internally calls
         * {@link #onBindViewHolder(ViewHolder, int)} to update the
         * {@link ViewHolder} contents with the item at the given position and
        ...
        public final void attachClickListener(VH holder, final View view, final int position) {
            ItemClickSupport itemClickSupport = null;
            if (holder != null && holder.itemView != null) {
                itemClickSupport = (ItemClickSupport) holder.itemView
                        .getTag(R.id.twowayview_item_click_support);
            }
            ...
            view.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (itemClick != null) {
                        itemClick.onItemSubViewClicked(view, position);
                    }
                }
            });
        }
         ...
         * unique identifier of type {@link java.lang.Long}.
         * 
         * @param hasStableIds Whether items in data set have unique identifiers
         *            or not.
         * @see #hasStableIds()
         * @see #getItemId(int)
         */
        ...
        /**
         * Returns the total number of items in the data set hold by the
         * adapter.
         * 
         ...
         * <p>
         * A view is recycled when a {@link LayoutManager} decides that it no
         * longer needs to be attached to its parent {@link RecyclerView}. This
         * can be because it has fallen out of visibility or a set of cached
         * views represented by views still attached to the parent RecyclerView.
         * If an item view has large or expensive data bound to it such as large
    ...
    public static abstract class LayoutManager {
        ...
        /**
         * Checks if RecyclerView is in the middle of a layout or scroll and
         * throws an {@link IllegalStateException} if it <b>is not</b>.
         * 
         * @param message The message for the exception. Can be null.
         * @see #assertNotInLayoutOrScroll(String)
         */
        ...
        /**
         * Checks if RecyclerView is in the middle of a layout or scroll and
         * throws an {@link IllegalStateException} if it <b>is</b>.
         * 
         * @param message The message for the exception. Can be null.
         * @see #assertInLayoutOrScroll(String)
         */
        ...
        /**
         * Returns whether this LayoutManager supports automatic item
         * animations. A LayoutManager wishing to support item animations should
         * obey certain rules as outlined in
         * {@link #onLayoutChildren(Recycler, State)}. The default return value
         * is <code>false</code>, so subclasses of LayoutManager will not get
         * predictive item animations by default.
         * <p>
         * Whether item animations are enabled in a RecyclerView is determined
         * both by the return value from this method and the
         * {@link RecyclerView#setItemAnimator(ItemAnimator) ItemAnimator} set
         * on the RecyclerView itself. If the RecyclerView has a non-null
         * ItemAnimator but this method returns false, then simple item
         * animations will be enabled, in which views that are moving onto or
         * off of the screen are simply faded in/out. If the RecyclerView has a
         * non-null ItemAnimator and this method returns true, then there will
         * be two calls to {@link #onLayoutChildren(Recycler, State)} to setup
         * up the information needed to more intelligently predict where
         * appearing and disappearing views should be animated from/to.
         * </p>
         * 
         * @return true if predictive item animations should be enabled, false
         *         otherwise
         */
        ...
        /**
         * @deprecated override
         *             {@link #onDetachedFromWindow(RecyclerView, Recycler)}
         */
        @Deprecated
         ...
         * 
         * @param view The RecyclerView this LayoutManager is bound to
         * @param recycler The recycler to use if you prefer to recycle your
         *            children instead of keeping them around.
         */
        ...
        /**
         * Check if the RecyclerView is configured to clip child views to its
         * padding.
         * 
         * @return true if this RecyclerView clips children to its padding,
         *         false otherwise
         */
        ...
        /**
         * Lay out all relevant child views from the given adapter. The
         * LayoutManager is in charge of the behavior of item animations. By
         * default, RecyclerView has a non-null {@link #getItemAnimator()
         * ItemAnimator}, and simple item animations are enabled. This means
         * that add/remove operations on the adapter will result in animations
         * to add new or appearing items, removed or disappearing items, and
         * moved items. If a LayoutManager returns false from
         * {@link #supportsPredictiveItemAnimations()}, which is the default,
         * and runs a normal layout operation during
         * {@link #onLayoutChildren(Recycler, State)}, the RecyclerView will
         * have enough information to run those animations in a simple way. For
         * example, the default ItemAnimator, {@link DefaultItemAnimator}, will
         * simple fade views in and out, whether they are actuall added/removed
         * or whether they are moved on or off the screen due to other
         * add/remove operations.
         * <p>
         * A LayoutManager wanting a better item animation experience, where
         * items can be animated onto and off of the screen according to where
         * the items exist when they are not on screen, then the LayoutManager
         * should return true from {@link #supportsPredictiveItemAnimations()}
         * and add additional logic to
         * {@link #onLayoutChildren(Recycler, State)}. Supporting predictive
         * animations means that {@link #onLayoutChildren(Recycler, State)} will
         * be called twice; once as a "pre" layout step to determine where items
         * would have been prior to a real layout, and again to do the "real"
         * layout. In the pre-layout phase, items will remember their pre-layout
         * positions to allow them to be laid out appropriately. Also,
         * {@link LayoutParams#isItemRemoved() removed} items will be returned
         * from the scrap to help determine correct placement of other items.
         * These removed items should not be added to the child list, but should
         * be used to help calculate correct positioning of other views,
         * including views that were not previously onscreen (referred to as
         * APPEARING views), but whose pre-layout offscreen position can be
         * determined given the extra information about the pre-layout removed
         * views.
         * </p>
         * <p>
         * The second layout pass is the real layout in which only non-removed
         * views will be used. The only additional requirement during this pass
         * is, if {@link #supportsPredictiveItemAnimations()} returns true, to
         * note which views exist in the child list prior to layout and which
         * are not there after layout (referred to as DISAPPEARING views), and
         * to position/layout those views appropriately, without regard to the
         * actual bounds of the RecyclerView. This allows the animation system
         * to know the location to which to animate these disappearing views.
         * </p>
         * <p>
         * The default LayoutManager implementations for RecyclerView handle all
         * of these requirements for animations already. Clients of RecyclerView
         * can either use one of these layout managers directly or look at their
         * implementations of onLayoutChildren() to see how they account for the
         ...
         * RecyclerView.
         * <p>
         * LayoutManagers will often want to use a custom
         * <code>LayoutParams</code> type to store extra information specific to
         * the layout. Client code should subclass
         ...
         * {@link #generateLayoutParams(android.view.ViewGroup.LayoutParams)}
         * and
        ...
        /**
         * Scroll to the specified adapter position. Actual position of the item
         * on the screen depends on the LayoutManager implementation.
         * 
         * @param position Scroll to this adapter position.
         */
        public void scrollToPosition(int position) {
         ...
         * 
         * @return {@link android.support.v4.view.ViewCompat#LAYOUT_DIRECTION_RTL}
         *         if the layout direction is RTL or returns
         *         {@link android.support.v4.view.ViewCompat#LAYOUT_DIRECTION_LTR}
        ...
        /**
         * To be called only during {@link #onLayoutChildren(Recycler, State)}
         * to add a view to the layout that is known to be going away, either
         * because it has been {@link Adapter#notifyItemRemoved(int) removed} or
         * because it is actually not in the visible portion of the container
         * but is being laid out in order to inform RecyclerView in how to
         * animate the item out of view.
         * <p>
         * Views added via this method are going to be invisible to
         * LayoutManager after the dispatchLayout pass is complete. They cannot
         * be retrieved via {@link #getChildAt(int)} or won't be included in
         * {@link #getChildCount()} method.
         * 
        ...
        private void addViewInt(View child, int index, boolean disappearing) {
            ...
            if (disappearing || holder.isRemoved()) {
                ...
                // This may look like unnecessary but may happen if layout
                // manager supports
                // predictive layouts and adapter removed then re-added the same
                // item.
                // In this case, added version will be visible in the post
                // layout (because add is
                // deferred) but RV will still bind it to the same View.
                // So if a View re-appears in post layout pass, remove it from
                // disappearing list.
            ...
            } else if (child.getParent() == mRecyclerView) { // it was not a
                                                             // scrap but a
                                                             // valid child
         ...
         * <p>
         * LayoutManagers may want to perform a lightweight detach operation to
         * rearrange views currently attached to the RecyclerView. Generally
         * LayoutManager implementations will want to use
         * {@link #detachAndScrapView(android.view.View, RecyclerView.Recycler)}
         * so that the detached view may be rebound and reused.
         * </p>
         * <p>
         * If a LayoutManager uses this method to detach a view, it
         * <em>must</em>
         * {@link #attachView(android.view.View, int, RecyclerView.LayoutParams)
         * reattach} or {@link #removeDetachedView(android.view.View) fully
         * remove} the detached view before the LayoutManager entry point method
         * called by RecyclerView returns.
         * </p>
         * 
        ...
        /**
         * Reattach a previously {@link #detachView(android.view.View) detached}
         ...
         * 
         * @return A direct child of RecyclerView which has focus or contains
         *         the focused child.
         */
         ...
         * {@link State#getItemCount()}. In methods where State is available,
         * you should use {@link State#getItemCount()} instead. For more
         ...
         * <p>
         * Scope of ignoring a child is strictly restricted to position
         * tracking, scrapping and recyling. Methods like
         * {@link #removeAndRecycleAllViews(Recycler)} will ignore the child
         * whereas {@link #removeAllViews()} or
         * {@link #offsetChildrenHorizontal(int)} will not ignore the child.
         * <p>
         * Before this child can be recycled again, you have to call
         * {@link #stopIgnoringView(View)}.
         * <p>
         * You can call this method only if your LayoutManger is in onLayout or
         * onScroll callback.
         * 
        ...
        public void ignoreView(View view) {
            if (view.getParent() != mRecyclerView || mRecyclerView.indexOfChild(view) == -1) {
                // checking this because calling this method on a recycled or
                // detached view may
                // cause loss of state.
         ...
         * <p>
         * Note that calling this method removes all information in the view
         * holder.
         * <p>
         * You can call this method only if your LayoutManger is in onLayout or
         * onScroll callback.
         ...
         *            or not. This method will invalidate RecyclerView if it
        ...
        /**
         * Calculates the item decor insets applied to the given child and
         * updates the provided Rect instance with the inset values.
         * <ul>
         ...
         * </ul>
         * <p>
         * Note that item decorations are automatically calculated when one of
         * the LayoutManager's measure child methods is called. If you need to
         * measure the child with custom specs via
         * {@link View#measure(int, int)}, you can use this method to get
         * decorations.
         * 
         ...
         * 
         * @param child Child to query
         * @return The total height of item decorations applied to the child's
         *         top.
        ...
        /**
         * Returns the total height of item decorations applied to child's
         * bottom.
         * <p>
         * Note that this value is not updated until the View is measured or
         * {@link #calculateItemDecorationsForChild(View, Rect)} is called.
         ...
         * 
         * @param child Child to query
         * @return The total width of item decorations applied to the child's
         *         left.
         ...
         *            {@link View#FOCUS_FORWARD} or 0 for not applicable
        ...
        /**
         * This method gives a LayoutManager an opportunity to intercept the
         * initial focus search before the default behavior of
         * {@link FocusFinder} is used. If this method returns null FocusFinder
         * will attempt to find a focusable child view. If it fails then
         * {@link #onFocusSearchFailed(View, int, RecyclerView.Recycler, RecyclerView.State)}
         * will be called to give the LayoutManager an opportunity to add new
         * views for items that did not have attached views representing them.
         ...
         * @return A descendant view to focus or null to fall back to default
         *         behavior. The default implementation returns null.
         */
         ...
         * {@link ViewParent#requestChildRectangleOnScreen(android.view.View, android.graphics.Rect, boolean)}
         * for more details.
         * <p>
        ...
        /**
         * @deprecated Use
         *             {@link #onRequestChildFocus(RecyclerView, State, View, View)}
         */
        @Deprecated
        ...
        /**
         * Called if the RecyclerView this LayoutManager is bound to has a
         * different adapter set. The LayoutManager may use this opportunity to
         * clear caches and configure state such that it can relayout
         * appropriately with the new data and potentially new view types.
         * <p>
         * The default implementation removes all currently attached views.
         * </p>
         * 
         * @param oldAdapter The previous adapter instance. Will be null if
         *            there was previously no adapter.
         * @param newAdapter The new adapter instance. Might be null if
         *            {@link #setAdapter(RecyclerView.Adapter)} is called with
         *            {@code null}.
         */
         ...
         * {@link ViewGroup#addFocusables(java.util.ArrayList, int)} should be
         * suppressed.
         * </p>
         * <p>
        ...
        /**
         * Called when {@link Adapter#notifyDataSetChanged()} is triggered
         ...
         * ADD/REMOVE/MOVE operation. This callback is only called if and only
         * if {@link Adapter#notifyItemMoved} is called.
         * 
         * @param recyclerView
         * @param from
         * @param to
         ...
         * </p>
         * <p>
         * Read {@link RecyclerView#computeHorizontalScrollExtent()} for
         * details.
         * </p>
         * <p>
         * Default implementation returns 0.
         ...
         * </p>
         * <p>
         * Read {@link RecyclerView#computeHorizontalScrollOffset()} for
         * details.
         * </p>
         * <p>
         * Default implementation returns 0.
         * </p>
         * 
         * @param state Current State of RecyclerView where you can find total
         *            item count
         * @return The horizontal offset of the scrollbar's thumb
         ...
         * @param state Current State of RecyclerView where you can find total
         *            item count
         * @return The total horizontal range represented by the vertical
         *         scrollbar
         ...
         * @param state Current State of RecyclerView where you can find total
         *            item count
         * @return The total vertical range represented by the vertical
         *         scrollbar
        ...
        /**
         * RecyclerView calls this method to notify LayoutManager that scroll
         * state has changed.
         * 
         ...
         * <p>
         * If you want to clean cached views as well, you should call
         * {@link Recycler#clear()} too.
         * <p>
        ...
        /**
         * Called by the AccessibilityDelegate when the information about the
         * current layout should be populated.
         * <p>
         * Default implementation adds a
         * {@link android.support.v4.view.accessibility.AccessibilityNodeInfoCompat.CollectionInfoCompat}.
         * <p>
         * You should override
         ...
         * {@link #isLayoutHierarchical(RecyclerView.Recycler, RecyclerView.State)}
         * and
         * {@link #getSelectionModeForAccessibility(RecyclerView.Recycler, RecyclerView.State)}
         * for more accurate accessibility information.
         * 
         * @param recycler The Recycler that can be used to convert view
         *            positions into adapter positions
         ...
         * @see #getRowCountForAccessibility(RecyclerView.Recycler,
         *      RecyclerView.State)
         * @see #getColumnCountForAccessibility(RecyclerView.Recycler,
         *      RecyclerView.State)
         * @see #isLayoutHierarchical(RecyclerView.Recycler, RecyclerView.State)
         * @see #getSelectionModeForAccessibility(RecyclerView.Recycler,
         *      RecyclerView.State)
         */
        ...
        /**
         * Called by the accessibility delegate to initialize an accessibility
         * event.
         * <p>
         * Default implementation adds item count and scroll information to the
         * event.
         * 
         * @param recycler The Recycler that can be used to convert view
         *            positions into adapter positions
        ...
        /**
         * Called by the AccessibilityDelegate when the accessibility
         * information for a specific item should be populated.
         * <p>
         * Default implementation adds basic positioning information about the
         * item.
         * 
         * @param recycler The Recycler that can be used to convert view
         *            positions into adapter positions
         * @param state The current state of RecyclerView
         * @param host The child for which accessibility node info should be
         *            populated
        ...
        public void onInitializeAccessibilityNodeInfoForItem(Recycler recycler, State state,
            ...
            final AccessibilityNodeInfoCompat.CollectionItemInfoCompat itemInfo = AccessibilityNodeInfoCompat.CollectionItemInfoCompat
                    .obtain(rowIndexGuess, 1,
                            columnIndexGuess, 1, false, false);
            info.setCollectionItemInfo(itemInfo);
        }

        /**
         * A LayoutManager can call this method to force RecyclerView to run
         * simple animations in the next layout pass, even if there is not any
         * trigger to do so. (e.g. adapter data change).
         * <p>
         * Note that, calling this method will not guarantee that RecyclerView
         * will run animations at all. For example, if there is not any
         * {@link ItemAnimator} set, RecyclerView will not run any animations
         * but will still clear this flag after the layout is complete.
         */
         ...
         * ,
         * {@link AccessibilityNodeInfoCompat.CollectionInfoCompat#SELECTION_MODE_SINGLE}
         * or
         ...
         *            positions into adapter positions
         * @param state The current state of RecyclerView
         * @return Selection mode for accessibility. Default implementation
         *         returns
         *         {@link AccessibilityNodeInfoCompat.CollectionInfoCompat#SELECTION_MODE_NONE}
         *         .
         */
         ...
         * LayoutManager supports vertical scrolling or 1 if LayoutManager does
         ...
         * <p>
         * Default implementation returns the number of items in the adapter if
         * LayoutManager supports horizontal scrolling or 1 if LayoutManager
        ...
        /**
         * Returns whether layout is hierarchical or not to be used for
         * accessibility.
         * <p>
        ...
        /**
         * Called by AccessibilityDelegate when an accessibility action is
         * requested on one of the chidren of LayoutManager.
         * <p>
     ...
     * item views (in
     * {@link ItemDecoration#onDraw(Canvas, RecyclerView, RecyclerView.State)
     * onDraw()} and after the items (in
     * {@link ItemDecoration#onDrawOver(Canvas, RecyclerView, RecyclerView.State)}
     * .
     * </p>
     */
    public static abstract class ItemDecoration {
        ...
        /**
         * @deprecated Override
         *             {@link #onDraw(Canvas, RecyclerView, RecyclerView.State)}
         */
        @Deprecated
        ...
        /**
         * @deprecated Override
         *             {@link #onDrawOver(Canvas, RecyclerView, RecyclerView.State)}
         */
        @Deprecated
        ...
        /**
         * @deprecated Use
         *             {@link #getItemOffsets(Rect, View, RecyclerView, State)}
         */
        @Deprecated
    ...
    public interface OnItemTouchListener {
         ...
         * </p>
         * 
         * @param e MotionEvent describing the touch event. All coordinates are
         *            in the RecyclerView's coordinate system.
    ...
    /**
     * An OnScrollListener can be set on a RecyclerView to receive messages when
     * a scrolling event has occurred on that RecyclerView.
     * 
     * @see RecyclerView#setOnScrollListener(OnScrollListener)
     */
    abstract static public class OnScrollListener {
        /**
         * Callback method to be invoked when RecyclerView's scroll state
         * changes.
         * 
         * @param recyclerView The RecyclerView whose scroll state has changed.
         * @param newState The updated scroll state. One of
         *            {@link #SCROLL_STATE_IDLE}, {@link #SCROLL_STATE_DRAGGING}
         *            or {@link #SCROLL_STATE_SETTLING}.
         */
    ...
    /**
     * A RecyclerListener can be set on a RecyclerView to receive messages
    ...
    public static abstract class ViewHolder {
        ...
        // The item that this holder is shadowing during an item change
        // event/animation
        ViewHolder mShadowedHolder = null;
        // The item that is shadowing this holder during an item change
        // event/animation
        ...
        /**
         * This ViewHolder is returned from scrap which means we are expecting
         * an addView call for this itemView. When returned from scrap,
         * ViewHolder stays in the scrap list until the end of the layout pass
         * and then recycled by RecyclerView if it is not added back to the
         * RecyclerView.
         */
        static final int FLAG_RETURNED_FROM_SCRAP = 1 << 5;

        /**
         * This ViewHolder's contents have changed. This flag is used as an
         * indication that change animations may be used, if supported by the
         * ItemAnimator.
         */
        static final int FLAG_CHANGED = 1 << 6;

        /**
         * This ViewHolder is fully managed by the LayoutManager. We do not
         * scrap, recycle or remove it unless LayoutManager is replaced. It is
         * still fully visible to the LayoutManager.
         */
        ...
        /**
         * When LayoutManager supports animations, RecyclerView tracks 3
         * positions for ViewHolders to perform animations.
         * <p>
         * If a ViewHolder was laid out in the previous onLayout call, old
         * position will keep its adapter index in the previous layout.
         * 
         * @return The previous adapter index of the Item represented by this
         *         ViewHolder or {@link #NO_POSITION} if old position does not
         *         exists or cleared (pre-layout is complete).
         */
         ...
         * 
         * @return The the item's id if adapter has stable ids,
         *         {@link RecyclerView#NO_ID} otherwise
         */
    ...
    /**
     * {@link android.view.ViewGroup.MarginLayoutParams LayoutParams} subclass
     * for children of {@link RecyclerView}. Custom {@link LayoutManager layout
     * managers} are encouraged to create their own subclass of this
     * <code>LayoutParams</code> class to store any additional required
     * per-child view metadata about the layout.
     */
    public static class LayoutParams extends android.view.ViewGroup.MarginLayoutParams {
        ViewHolder mViewHolder;
        final Rect mDecorInsets = new Rect();
        boolean mInsetsDirty = true;
        // Flag is set to true if the view is bound while it is detached from
        // RV.
        // In this case, we need to manually call invalidate after view is added
        // to guarantee that
        ...
        /**
         * Returns true if the adapter data item corresponding to the view this
         * LayoutParams is attached to has been changed in the data set. A
         * LayoutManager may choose to treat it differently in order to animate
         * its changing state.
         * 
         * @return true if the item the view corresponds to was changed in the
         *         data set
         */
    ...
    /**
     * <p>
     * Base class for smooth scrolling. Handles basic tracking of the target
    ...
    public static abstract class SmoothScroller {
         ...
         * <p>
         * In each animation step, {@link RecyclerView} will check for the
         * target view and call either
         * {@link #onTargetFound(android.view.View, RecyclerView.State, SmoothScroller.Action)}
         * or
         * {@link #onSeekTargetStep(int, int, RecyclerView.State, SmoothScroller.Action)}
         * until SmoothScroller is stopped.
         * </p>
         * <p>
         * Note that if RecyclerView finds the target view, it will
         * automatically stop the SmoothScroller. This <b>does not</b> mean that
         * scroll will stop, it only means it will stop calling SmoothScroller
        ...
        /**
         * Stops running the SmoothScroller in each animation callback. Note
         * that this does not cancel any existing {@link Action} updated by
         * {@link #onTargetFound(android.view.View, RecyclerView.State, SmoothScroller.Action)}
         * or
         * {@link #onSeekTargetStep(int, int, RecyclerView.State, SmoothScroller.Action)}
         * .
         ...
         * update the provided {@link Action} to define the next scroll.
         * </p>
         * 
         ...
         *            the previous one, update this object.
         */
         ...
         * callback SmoothScroller will receive and it should update the
         * provided {@link Action} to define the scroll details towards the
         * target view.
         * 
         * @param targetView The view element which render the target position.
         * @param state Transient state of RecyclerView
         * @param action Action instance that you should update to define final
        ...
        public static class Action {
             ...
             *            scroll position in each animation step
             */
                ...
                if (changed) {
                    ...
                    if (mInterpolator == null) {
                        ...
                        recyclerView.mViewFlinger
                                .smoothScrollBy(mDx, mDy, mDuration, mInterpolator);
                    }
                    consecutiveUpdates++;
    ...
    public static class State {
        ...
        /**
         * Returns whether RecyclerView will run predictive animations in this
         * layout pass or not.
         * 
         * @return true if RecyclerView is calculating predictive animations to
         *         be run at the end of the layout pass.
         */
        ...
        /**
         * Returns whether RecyclerView will run simple animations in this
         * layout pass or not.
         * 
         * @return true if RecyclerView is calculating simple animations to be
         *         run at the end of the layout pass.
         */
         ...
         *            suggested to use R.id.* to preserve cross functionality
         *            and avoid conflicts.
         */
         ...
         *            to use R.id.* to preserve cross functionality and avoid
         *            conflicts.
        ...
        /**
         * If scroll is triggered to make a certain item visible, this value
         * will return the adapter index of that item.
         * 
        ...
        /**
         * Returns the total number of items that can be laid out. Note that
         * this number is not necessarily equal to the number of items in the
         * adapter, so you should always use this number for your position
         * calculations and never access the adapter directly.
         * <p>
         * RecyclerView listens for Adapter's notify events and calculates the
         * effects of adapter data changes on existing Views. These calculations
         * are used to decide which animations should be run.
         * <p>
         * To support predictive animations, RecyclerView may rewrite or reorder
         * Adapter changes to present the correct state to LayoutManager in
         * pre-layout pass.
         * <p>
         * For example, a newly added item is not included in pre-layout item
         * count because pre-layout reflects the contents of the adapter before
         * the item is added. Behind the scenes, RecyclerView offsets
         * {@link Recycler#getViewForPosition(int)} calls such that
         * LayoutManager does not know about the new item's existence in
         * pre-layout. The item will be available in second layout pass and will
         * be included in the item count. Similar adjustments are made for moved
         * and removed items as well.
         * <p>
         * You can get the adapter's item count via
         * {@link LayoutManager#getItemCount()} method.
         * 
    ...
    /**
     * Internal listener that manages items after animations finish. This is how
     * items are retained (not recycled) during animations, but allowed to be
     * recycled afterwards. It depends on the contract with the ItemAnimator to
     * call the appropriate dispatch*Finished() method on the animator's
     * listener when it is done animating any item.
     */
    private class ItemAnimatorRestoreListener implements ItemAnimator.ItemAnimatorListener {
        ...
        public void onChangeFinished(ViewHolder item) {
            item.setIsRecyclable(true);
            /**
             * We check both shadowed and shadowing because a ViewHolder may get
             * both roles at the same time. Assume this flow: item X is
             * represented by VH_1. Then itemX changes, so we create VH_2 . RV
             * sets the following and calls item animator: VH_1.shadowed = VH_2;
             * VH_1.mChanged = true; VH_2.shadowing =VH_1; Then, before the
             * first change finishes, item changes again so we create VH_3. RV
             * sets the following and calls item animator: VH_2.shadowed = VH_3
             * VH_2.mChanged = true VH_3.shadowing = VH_2 Because VH_2 already
             * has an animation, it will be cancelled. At this point VH_2 has
             * both shadowing and shadowed fields set. Shadowing information is
             * obsolete now because the first animation where VH_2 is
             * newViewHolder is not valid anymore. We ended up in this case
             * because VH_2 played both roles. On the other hand, we DO NOT want
             * to clear its changed flag. If second change was simply reverting
             * first change, we would find VH_1 in
             * {@link Recycler#getScrapViewForPosition(int, int, boolean)} and
             * recycle it before re-using
             */
            if (item.mShadowedHolder != null && item.mShadowingHolder == null) { // old
                                                                                 // vh
                item.mShadowedHolder = null;
                item.setFlags(~ViewHolder.FLAG_CHANGED, item.mFlags);
            }
            // always null this because an OldViewHolder can never become
            // NewViewHolder w/o being
            // recycled.
    ...
    /**
     * This class defines the animations that take place on items as changes are
     * made to the adapter. Subclasses of ItemAnimator can be used to implement
     * custom animations for actions on ViewHolder items. The RecyclerView will
     * manage retaining these items while they are being animated, but
     * implementors must call the appropriate "Starting" (
     * {@link #dispatchRemoveStarting(ViewHolder)},
     * {@link #dispatchMoveStarting(ViewHolder)},
     * {@link #dispatchChangeStarting(ViewHolder, boolean)}, or
     * {@link #dispatchAddStarting(ViewHolder)}) and "Finished" (
     * {@link #dispatchRemoveFinished(ViewHolder)},
     * {@link #dispatchMoveFinished(ViewHolder)},
     * {@link #dispatchChangeFinished(ViewHolder, boolean)}, or
     * {@link #dispatchAddFinished(ViewHolder)}) methods when each item
     * animation is being started and ended.
     * <p>
    ...
    public static abstract class ItemAnimator {
        ...
        /**
         * Returns whether this ItemAnimator supports animations of change
         * events.
         * 
        ...
        /**
         * Sets whether this ItemAnimator supports animations of item change
         * events. By default, ItemAnimator only supports animations when items
         * are added or removed. By setting this property to true, actions on
         * the data set which change the contents of items may also be animated.
         * What those animations are is left up to the discretion of the
         * ItemAnimator subclass, in its
         * {@link #animateChange(ViewHolder, ViewHolder, int, int, int, int)}
         * implementation. The value of this property is false by default.
         * 
         * @see Adapter#notifyItemChanged(int)
         * @see Adapter#notifyItemRangeChanged(int, int)
         * @param supportsChangeAnimations true if change animations are
         *            supported by this ItemAnimator, false otherwise. If the
         *            property is false, the ItemAnimator will not receive a
         *            call to
         *            {@link #animateChange(ViewHolder, ViewHolder, int, int, int, int)}
         *            when changes occur.
         */
        ...
        /**
         * Internal only: Sets the listener that must be called when the
         * animator is finished animating the item (or immediately if no
         * animation happens). This is set internally and is not intended to be
        ...
        /**
         * Called when there are pending animations waiting to be started. This
         * state is governed by the return values from
         * {@link #animateAdd(ViewHolder) animateAdd()},
         * {@link #animateMove(ViewHolder, int, int, int, int) animateMove()},
         * and {@link #animateRemove(ViewHolder) animateRemove()}, which inform
         * the RecyclerView that the ItemAnimator wants to be called later to
         * start the associated animations. runPendingAnimations() will be
         * scheduled to be run on the next frame.
         */
        abstract public void runPendingAnimations();

        /**
         * Called when an item is removed from the RecyclerView. Implementors
         * can choose whether and how to animate that change, but must always
         * call {@link #dispatchRemoveFinished(ViewHolder)} when done, either
         * immediately (if no animation will occur) or after the animation
         * actually finishes. The return value indicates whether an animation
         * has been set up and whether the ItemAnimator's
         * {@link #runPendingAnimations()} method should be called at the next
         * opportunity. This mechanism allows ItemAnimator to set up individual
         * animations as separate calls to {@link #animateAdd(ViewHolder)
         * animateAdd()}, {@link #animateMove(ViewHolder, int, int, int, int)
         * animateMove()}, {@link #animateRemove(ViewHolder) animateRemove()},
         * and
         * {@link #animateChange(ViewHolder, ViewHolder, int, int, int, int)}
         * come in one by one, then start the animations together in the later
         * call to {@link #runPendingAnimations()}.
         * <p>
         * This method may also be called for disappearing items which continue
         * to exist in the RecyclerView, but for which the system does not have
         * enough information to animate them out of view. In that case, the
         * default animation for removing items is run on those items as well.
         * </p>
         * 
         * @param holder The item that is being removed.
         * @return true if a later call to {@link #runPendingAnimations()} is
         *         requested, false otherwise.
         */
        abstract public boolean animateRemove(ViewHolder holder);

        /**
         * Called when an item is added to the RecyclerView. Implementors can
         * choose whether and how to animate that change, but must always call
         ...
         * call to {@link #runPendingAnimations()}.
         * <p>
         * This method may also be called for appearing items which were already
         * in the RecyclerView, but for which the system does not have enough
         * information to animate them into view. In that case, the default
         * animation for adding items is run on those items as well.
         * </p>
         * 
        ...
        /**
         * Called when an item is moved in the RecyclerView. Implementors can
         * choose whether and how to animate that change, but must always call
        ...
        /**
         * Called when an item is changed in the RecyclerView, as indicated by a
         * call to {@link Adapter#notifyItemChanged(int)} or
         * {@link Adapter#notifyItemRangeChanged(int, int)}.
         * <p>
         * Implementers can choose whether and how to animate changes, but must
         * always call {@link #dispatchChangeFinished(ViewHolder, boolean)} for
         * each non-null ViewHolder, either immediately (if no animation will
         * occur) or after the animation actually finishes. The return value
         * indicates whether an animation has been set up and whether the
         * ItemAnimator's {@link #runPendingAnimations()} method should be
         * called at the next opportunity. This mechanism allows ItemAnimator to
         * set up individual animations as separate calls to
         * {@link #animateAdd(ViewHolder) animateAdd()},
         * {@link #animateMove(ViewHolder, int, int, int, int) animateMove()},
         ...
         * @param newHolder The new item that was created with the changed
         ...
         * @param item The item which has been changed (this method must be
         *            called for each non-null ViewHolder passed into
         *            {@link #animateChange(ViewHolder, ViewHolder, int, int, int, int)}
         *            ).
         * @param oldItem true if this is the old item that was changed, false
         *            if it is the new item that replaced the old item.
         */
        ...
        /**
         * Method to be called by subclasses when a remove animation is being
         * started.
         * 
        ...
        /**
         * Method to be called by subclasses when a move animation is being
         * started.
         * 
        ...
        /**
         * Method to be called by subclasses when an add animation is being
         * started.
         * 
        ...
        /**
         * Method to be called by subclasses when a change animation is being
         * started.
         * 
         * @param item The item which has been changed (this method must be
         *            called for each non-null ViewHolder passed into
        ...
        /**
         * Method called when an animation on a view should be ended
         * immediately. This could happen when other events, like scrolling,
         * occur, so that animating views can be quickly put into their proper
         * end locations. Implementations should ensure that any animations
         * running on the item are canceled and affected properties are set to
         * their end values. Also, appropriate dispatch methods (e.g.,
         * {@link #dispatchAddFinished(ViewHolder)} should be called since the
         * animations are effectively done when this method is called.
         * 
         ...
         * Implementations should ensure that any animations running on any
         * items are canceled and affected properties are set to their end
         * values. Also, appropriate dispatch methods (e.g.,
         * {@link #dispatchAddFinished(ViewHolder)} should be called since the
         * animations are effectively done when this method is called.
         */
        ...
        /**
         * Method which returns whether there are any item animations currently
         * running. This method can be used to determine whether to delay other
         * actions until animations end.
         * 
         * @return true if there are any item animations currently running,
         *         false otherwise.
         */
        abstract public boolean isRunning();

        /**
         * Like {@link #isRunning()}, this method returns whether there are any
         * item animations currently running. Addtionally, the listener passed
         * in will be called when there are no item animations running, either
         * immediately (before the method returns) if no animations are
         * currently running, or when the currently running animations are
         * {@link #dispatchAnimationsFinished() finished}.
         * <p>
         * Note that the listener is transient - it is either called immediately
         * and not stored at all, or stored only until it is called when running
         * animations are finished sometime later.
         * </p>
         * 
         * @param listener A listener to be called immediately if no animations
         *            are running or later when currently-running animations
         *            have finished. A null listener is equivalent to calling
         *            {@link #isRunning()}.
         * @return true if there are any item animations currently running,
         *         false otherwise.
         */
        ...
        /**
         * The interface to be implemented by listeners to animation events from
         * this ItemAnimator. This is used internally and is not intended for
         * developers to create directly.
         */
        ...
        /**
         * This method should be called by ItemAnimator implementations to
         * notify any listeners that all pending and active item animations are
         * finished.
         */
        ...
        /**
         * This interface is used to inform listeners when all pending or
         * running animations in an ItemAnimator are finished. This can be used,
         * for example, to delay an action in a data set until currently-running
        ...
        /**
         * Called when a remove animation is being started on the given
         * ViewHolder. The default implementation does nothing. Subclasses may
         * wish to override this method to handle any ViewHolder-specific
         * operations linked to animation lifecycles.
         * 
        ...
        /**
         * Called when a remove animation has ended on the given ViewHolder. The
         * default implementation does nothing. Subclasses may wish to override
         * this method to handle any ViewHolder-specific operations linked to
        ...
        /**
         * Called when an add animation is being started on the given
         * ViewHolder. The default implementation does nothing. Subclasses may
         * wish to override this method to handle any ViewHolder-specific
         * operations linked to animation lifecycles.
         * 
        ...
        /**
         * Called when an add animation has ended on the given ViewHolder. The
         * default implementation does nothing. Subclasses may wish to override
         * this method to handle any ViewHolder-specific operations linked to
         * animation lifecycles.
         * 
        ...
        /**
         * Called when a move animation is being started on the given
         * ViewHolder. The default implementation does nothing. Subclasses may
         * wish to override this method to handle any ViewHolder-specific
         * operations linked to animation lifecycles.
         * 
        ...
        /**
         * Called when a move animation has ended on the given ViewHolder. The
         * default implementation does nothing. Subclasses may wish to override
         * this method to handle any ViewHolder-specific operations linked to
         * animation lifecycles.
         * 
        ...
        /**
         * Called when a change animation is being started on the given
         * ViewHolder. The default implementation does nothing. Subclasses may
         * wish to override this method to handle any ViewHolder-specific
         * operations linked to animation lifecycles.
         * 
        ...
        /**
         * Called when a change animation has ended on the given ViewHolder. The
         * default implementation does nothing. Subclasses may wish to override
         * this method to handle any ViewHolder-specific operations linked to
         * animation lifecycles.
         * 
