 ...
 * <p>
 * To support animations, RecyclerView presents an older version the Adapter to
 * best represent previous state of the layout. Sometimes, this is not trivial
 * when items are removed that were not laid out, in which case, RecyclerView
 * has no way of providing that item's view for animations.
 * <p>
 * AdapterHelper creates an UpdateOp for each adapter data change then
 * pre-processes them. During pre processing, AdapterHelper finds out which
 * UpdateOps can be deferred to second layout pass and which cannot. For the
 * UpdateOps that cannot be deferred, AdapterHelper will change them according
 * to previously deferred operation and dispatch them before the first layout
 * pass. It also takes care of updating deferred UpdateOps since order of
 * operations is changed by this process.
 * <p>
 * Although operations may be forwarded to LayoutManager in different orders,
 * resulting data set is guaranteed to be the consistent.
 */
class AdapterHelper implements OpReorderer.Callback {
    ...
    private void applyMove(UpdateOp op) {
        // MOVE ops are pre-processed so at this point, we know that item is
    ...
    private void applyRemove(UpdateOp op) {
        ...
        for (int position = op.positionStart; position < tmpEnd; position++) {
            ...
            if (vh != null || canFindInPreLayout(position)) {
                // If a ViewHolder exists or this is a newly added item, we can
                // defer this update
                // to post layout stage.
                // * For existing ViewHolders, we'll fake its existence in the
                // pre-layout phase.
                // * For items that are added and removed in the same process
                // cycle, they won't
                // have any effect in pre-layout since their add ops are already
                // deferred to
                // post-layout pass.
                if (type == POSITION_TYPE_INVISIBLE) {
                    // Looks like we have other updates that we cannot merge
                    // with this one.
                ...
                // This update cannot be recovered because we don't have a
                // ViewHolder representing
    ...
    private void dispatchAndUpdateViewHolders(UpdateOp op) {
        // tricky part.
        // traverse all postpones and revert their changes on this op if
        // necessary, apply updated
        ...
        // handle each pos 1 by 1 to ensure continuity. If it breaks, dispatch
        // partial
        // TODO Since move ops are pushed to end, we should not need this
        // anymore
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (postponed.cmd == UpdateOp.MOVE) {
                ...
                if (pos >= start && pos <= end) {
                    ...
                    // postponed MV is outside the dispatched OP. if it is
                    // before, offset
                    if (cmd == UpdateOp.ADD) {
                        postponed.positionStart++;
                        postponed.itemCount++;
    ...
    void consumeUpdatesInOnePass() {
        // we still consume postponed updates (if there is) in case there was a
        // pre-process call
