...
public class LinearLayoutManager extends RecyclerView.LayoutManager {
    ...
    /**
     * While trying to find next view to focus, LayoutManager will not try to
     * scroll more than this factor times the total space of the list. If layout
     * is vertical, total space is the height minus padding, if layout is
    ...
    /**
     * Helper class that keeps temporary layout state. It does not keep state
     * after layout is complete but we still keep a reference to re-use the same
     * object.
     */
     ...
     * this interface helps {@link LinearLayoutManager} make those decisions.
    ...
    /**
     * Re-used variable to keep anchor information on re-layout. Anchor position
     * and coordinate defines the reference point for LLM while doing a layout.
     */
    ...
    /**
     * Returns whether LayoutManager will recycle its children when it is
     * detached from RecyclerView.
     * 
     * @return true if LayoutManager will recycle its children when it is
     *         detached from RecyclerView.
     */
    ...
    /**
     * Set whether LayoutManager will recycle its children when it is detached
     * from RecyclerView.
     * <p>
     * If you are using a {@link RecyclerView.RecycledViewPool}, it might be a
     * good idea to set this flag to <code>true</code> so that views will be
     * avilable to other RecyclerViews immediately.
     * <p>
     * Note that, setting this flag will result in a performance drop if
     * RecyclerView is restored.
     * 
     * @param recycleChildrenOnDetach Whether children should be recycled in
     *            detach or not.
     */
     ...
     * the layout change for RTL views. When set to true, first item is laid out
     * at the end of the UI, second item is laid out before it etc. For
     * horizontal layouts, it depends on the layout direction. When set to true,
     * If {@link android.support.v7.widget.RecyclerView} is LTR, than it will
     * layout from RTL, if {@link android.support.v7.widget.RecyclerView} is
     * RTL, it will layout from LTR. If you are looking for the exact same
     * behavior of
     ...
     * </p>
     * <p>
     * You can also use this if you are trying to pre-layout your upcoming
     * views.
     * </p>
     * 
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        // LLM may decide to layout items for "extra" pixels to account for
        // scrolling target,
        ...
        if (state.isPreLayout() && mPendingScrollPosition != NO_POSITION &&
            ...
            // if the child is visible and we are going to move it around, we
            // should layout
            // extra items in the opposite direction to make sure new items
            // animate nicely
        ...
        // TODO we can probably avoid this if neither
        // stackFromEnd/reverseLayout/RTL values have
        // changed
        if (getChildCount() > 0) {
     ...
     * 
     * @param state
     * @param anchorInfo Simple data structure to keep anchor point information
     *            for the next layout
     */
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler,
        ...
        // to make the logic simpler, we calculate the size of children and call
        // fill.
    ...
    /**
     * Finds an anchor child from existing Views. Most of the time, this is the
     * view closest to start or end that has a valid position (e.g. not
     * removed).
     * <p>
    ...
    private boolean updateAnchorFromChildren(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (referenceChild != null) {
            ...
            // If all visible views are removed in 1 pass, reference child might
            // be out of bounds.
            // If that is the case, offset it back to 0 so that we use these
            // pre-layout children.
            if (!state.isPreLayout() && supportsPredictiveItemAnimations()) {
                // validate this child is at least partially visible. if not,
                // offset it to start
     ...
     * info from that data and returns true
     */
    private boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        // if child is visible, try to make it a reference child and ensure it
        // is fully visible.
        // if child is not visible, align it depending on its virtual position.
        anchorInfo.mPosition = mPendingScrollPosition;
        if (mPendingSavedState != null && mPendingSavedState.hasValidAnchor()) {
            // Anchor offset depends on how that child was laid out. Here, we
            // update it
     ...
     * resolved layout start. Resolved layout start depends on
     * {@link #getReverseLayout()},
     * {@link ViewCompat#getLayoutDirection(android.view.View)} and
     * {@link #getStackFromEnd()}.
     * <p>
     * For example, if layout is {@link #VERTICAL} and
     * {@link #getStackFromEnd()} is true, calling
     * <code>scrollToPositionWithOffset(10, 20)</code> will layout such that
     * <code>item[10]</code>'s bottom is 20 pixels above the RecyclerView's
     * bottom.
     * <p>
     * Note that scroll position change will not be reflected until the next
     * layout call.
     ...
     * items. This however assumes that all list items have similar or equal
     * widths or heights (depending on list orientation). If you use a list in
     * which items have different dimensions, the scrollbar will change
     * appearance as the user scrolls through the list. To avoid this issue, you
     * need to disable this property. When smooth scrollbar is disabled, the
     * position and size of the scrollbar thumb is based solely on the number of
     * items in the adapter and the position of the visible items inside the
     * adapter. This provides a stable scrollbar as the user navigates through a
     * list of items with varying widths / heights.
     * 
    ...
    /**
     * Returns the current state of the smooth scrollbar feature. It is enabled
     * by default.
     * 
     ...
     * @param dt This can be used to add additional padding to the visible area.
     *            This is used to detect children that will go out of bounds
     *            after scrolling, without actually moving them.
     */
     ...
     * 
     * @param focusDirection One of {@link View#FOCUS_UP},
     ...
     * @return {@link LayoutState#LAYOUT_START} or
     *         {@link LayoutState#LAYOUT_END} if focus direction is applicable
     *         to current state, {@link LayoutState#INVALID_LAYOUT} otherwise.
     */
     ...
     * returns the one closest to the end of the layout.
     * <p>
     * Due to ambiguous adapter updates or children being removed, some
     * children's positions may be invalid. This method is a best effort to find
     * a position within adapter bounds if possible.
     * <p>
    ...
    /**
     * Among the children that are suitable to be considered as an anchor child,
     * returns the one closest to the start of the layout.
     * <p>
     * Due to ambiguous adapter updates or children being removed, some
     * children's positions may be invalid. This method is a best effort to find
     * a position within adapter bounds if possible.
            ...
            if (position >= 0 && position < itemCount) {
                if (((RecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
                    ...
                    if (outOfBoundsMatch == null) {
                        outOfBoundsMatch = view; // item is not visible, less
                                                 // preferred
                    }
                } else {
                    return view;
     ...
     * Those views are ignored in this method.
     * 
     ...
     * means, if LayoutManager is horizontal, it will only check the view's left
     * and right edges.
     * 
     ...
     * constraint. In default layout, child 0 should be closest to screen
     * position 0 and last child should be closest to position WIDTH or HEIGHT.
     * In reverse layout, last child should be closes to screen position 0 and
     * first child should be closest to position WIDTH or HEIGHT
     */
    ...
    static class LayoutState {
         ...
         * laid out for {@link #mExtra} is not considered to avoid recycling
         * visible children.
         */
        int mExtra = 0;

        /**
         * Equal to {@link RecyclerView.State#isPreLayout()}. When consuming
         * scrap, if this value is set to true, we skip removed views since they
         * should not be laid out in post layout step.
         */
        boolean mIsPreLayout = false;

        /**
         * When LLM needs to layout particular views, it sets this list in which
         * case, LayoutState will only return views from this list and return
        ...
        /**
         * Gets the view for the next element that we should layout. Also
    ...
    class AnchorInfo {
        ...
        /**
         * assigns anchor coordinate from the RecyclerView's padding depending
         * on current layoutFromEnd value
         */
        ...
        /**
         * Assign anchor position information from the provided view if it is
         * valid as a reference child.
         */
