...
public class DownloadReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        } else if (DownloadManager.ACTION_DOWNLOAD_INFORMATION.equals(action)) {
            handleDownloadInformation(context,
                    intent.getIntExtra(DownloadManager.EXTRA_DOWNLOAD_STATUS, -1));
        }
    }
    ...
    private void handleDownloadInformation(Context context, int status) {
        if (null == context) {
            return;
        }

        if (Downloads.Impl.STATUS_WAITING_FOR_NETWORK == status) {
            Toast.makeText(context, context.getString(R.string.network_unavailable),
