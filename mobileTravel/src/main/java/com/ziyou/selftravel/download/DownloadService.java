...
import com.ziyou.selftravel.download.Downloads.Impl;
import com.ziyou.selftravel.model.IOfflinePackage;
import com.ziyou.selftravel.util.CommonUtils;
import com.ziyou.selftravel.util.FileUtils;
import com.ziyou.selftravel.util.IndentingPrintWriter;
...
import static com.ziyou.selftravel.download.Constants.TAG;
...
public class DownloadService extends Service {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                if (info.mDeleted) {
                    ...
                    final boolean activeScan = false;
                    unzipPackageIfReady(info);
    ...
    private void unzipPackageIfReady(DownloadInfo info) {
        if (info.mStatus != Downloads.Impl.STATUS_SUCCESS) {
            return;
        }

        Log.d(TAG, "unzipPackage, DownloadInfo=" + info);
        Uri fileUri = Uri.parse(info.mFileName);
        Uri downloadUri = Uri.parse(info.mUri);

        if (fileUri == null || downloadUri == null) {
            Log.e(TAG, "Invalid fileUri:" + fileUri);
            return;
        }
        ...
        File dest = new File(OfflinePackageManager.getPackageDirFromUri(downloadUri));
        if (!dest.exists()) {
            try {
                ZipFile zipFile = new ZipFile(file);
                zipFile.extractAll(dest.getAbsolutePath());
                preventMediaScanner(dest);
                Log.d(TAG, "File successfully unzipped to " + dest.getAbsolutePath());
            ...
            Log.d(TAG, "unzip dest dir " + dest.getAbsolutePath() + " already exists, skipping");
        }

        ContentValues values = new ContentValues();
        values.put(Impl.COLUMN_CONTENT_PATH, Uri.fromFile(dest).toString());
        CommonUtils.assertTrue(getContentResolver().update(info.getAllDownloadsUri(),
                values,
                null, null) == 1);
    }

    private void preventMediaScanner(File offlinePackagePath) {
        List<File> multimediaFiles = new ArrayList<File>();
        multimediaFiles.add(new File(offlinePackagePath, IOfflinePackage.AUDIO_PATH));
        multimediaFiles.add(new File(offlinePackagePath, IOfflinePackage.VIDEO_PATH));
        multimediaFiles.add(new File(offlinePackagePath, IOfflinePackage.IMAGE_PATH));

        try {
            for (File file : multimediaFiles) {
                if (file.exists()) {
                    FileUtils.stringToFile(new File(file, ".nomedia").getAbsolutePath(), "");
                    Log.d(TAG, "preventMediaScanner from " + file);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
