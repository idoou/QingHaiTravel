...
import static com.ziyou.selftravel.download.Constants.LOGV;
...
/**
 * Manages the storage space consumed by Downloads Data dir. When space falls
 * below a threshold limit (set in resource xml files), starts cleanup of the
 * Downloads data dir to free up space.
 */
class StorageManager {
    /** the max amount of space allowed to be taken up by the downloads data dir */
    private static final long sMaxdownloadDataDirSize = 500 * 1024 * 1024;
     ...
     */
    private static final long sDownloadDataDirLowSpaceThreshold = 500 * 1024 * 1024;
    ...
    /**
     * how often do we need to perform checks on space to make sure space is
     * available
     */
    ...
    /**
     * How often should database and filesystem be cleaned up to remove spurious
     * files from the file system and The value is specified in terms of num of
     * downloads since last time the cleanup was done.
     */
    ...
    /*
     * start a thread to cleanup the following remove spurious files from the
     * file system remove excess entries from the database
     */
    ...
    /**
     * finds space in the given filesystem (input param: root) to accommodate #
     * of bytes specified by the input param(targetBytes). returns true if
    ...
    private synchronized void findSpace(File root, long targetBytes, int destination)
        ...
        if (root.equals(mDownloadDataDir)) {
            // this download is going into downloads data dir. check space in
            // that specific dir.
    ...
    /**
     * returns the number of bytes available in the downloads data dir TODO this
     * implementation is too slow. optimize it.
     */
        ...
        switch (destination) {
            ...
            case Downloads.Impl.DESTINATION_EXTERNAL:
                ...
                if (!base.isDirectory() && !base.mkdir()) {
                    // Can't create download directory, e.g. because a file
                    // called "download"
                    // already exists at the root level, or the SD card
    ...
    /**
     * Removes files in the systemcache and downloads data dir without
     * corresponding entries in the downloads database. This can occur if a
     * delete is done on the database but the file is not removed from the
     * filesystem (due to sudden death of the process, for example). This is not
     * a very common occurrence. So, do this only once in a while.
     */
     ...
     * TODO logic in this method needs to be optimized. maintain the number of
     * downloads in memory - so that this method can limit the amount of data
     * read.
     */
    private void trimDatabase() {
        ...
        try {
            cursor = mContext.getContentResolver().query(Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI,
                    ...
                    Downloads.Impl.COLUMN_LAST_MODIFICATION);
            if (cursor == null) {
                // This isn't good - if we can't do basic queries in our
                // database,
            ...
            // trimming the database raised an exception. alright, ignore the
            // exception
            // and return silently. trimming database is not exactly a critical
            // operation
