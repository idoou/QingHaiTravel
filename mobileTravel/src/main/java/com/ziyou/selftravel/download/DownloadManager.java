 ...
 * {@link android.content.Context#DOWNLOAD_SERVICE}. Apps that request downloads
 * through this API should register a broadcast receiver for
 * {@link #ACTION_NOTIFICATION_CLICKED} to appropriately handle when the user
 * clicks on a running download in a notification or from the downloads UI. Note
 * that the application must have the
 * {@link android.Manifest.permission#INTERNET} permission to use this class.
 */
public class DownloadManager {
     ...
     * on the value of {@link #COLUMN_STATUS}. When {@link #COLUMN_STATUS} is
     * {@link #STATUS_FAILED}, this indicates the type of error that occurred.
     * If an HTTP error occurred, this will hold the HTTP status code as defined
     * in RFC 2616. Otherwise, it will hold one of the ERROR_* constants. When
     * {@link #COLUMN_STATUS} is {@link #STATUS_PAUSED}, this indicates why the
     * download is paused. It will hold one of the PAUSED_* constants. If
     ...
     * completes.
     */
    public final static String ACTION_DOWNLOAD_COMPLETE = "com.ziyou.selftravel.intent.action.DOWNLOAD_COMPLETE";

    /**
     * Broadcast intent action sent by the download manager.
     * information.
     */
    public final static String ACTION_DOWNLOAD_INFORMATION = "com.ziyou.selftravel.intent.action.DOWNLOAD_INFORMATION";
    ...
    public final static String ACTION_NOTIFICATION_CLICKED =
            "com.ziyou.selftravel.intent.action.DOWNLOAD_NOTIFICATION_CLICKED";

    /**
     * Intent action to launch an activity to display all downloads.
     */
    public final static String ACTION_VIEW_DOWNLOADS = "com.ziyou.selftravel.intent.action.VIEW_DOWNLOADS";
    ...
    /**
     * Intent extra included with {@link #ACTION_DOWNLOAD_INFORMATION} intents,
     */
     ...
     * download. The URI is the only required parameter. Note that the default
     * download destination is a shared volume where the system might delete
     * your file if it needs to reclaim space for system use. If this is a
     * problem, use a location on external storage (see
    ...
    public static class Request {
        ...
        public Request(ScenicDetails scenicDetails) {
            this(Uri.parse(scenicDetails.offlinePackage.url));
            mScenicDetails = scenicDetails;
        }
         ...
         * <p>
         * By default, downloads are saved to a generated filename in the shared
         * download cache and may be deleted by the system at any time to
         * reclaim space.
         * 
         * @return this object
         */
        ...
        /**
         * Set the local destination for the downloaded file to the system cache
         * dir (/cache). This is only available to System apps with the
         * permission
         ...
         * made scannable by calling {@link #allowScanningByMediaScanner()}.
         * <p>
         * Files downloaded to /cache may be deleted by the system at any time
         * to reclaim space.
         * 
         * @return this object
         * @hide
         ...
         * {@link android.app.NotificationManager}. By default, a notification
         * is shown. If set to false, this requires the permission
        ...
        ContentValues toContentValues(String packageName) {
            ...
            values.put(Downloads.Impl.COLUMN_SCENIC_ID, mScenicDetails.id());
            values.put(Downloads.Impl.COLUMN_SCENIC_INTRO, mScenicDetails.introTitle);
            values.put(Downloads.Impl.COLUMN_SCENIC_NAME, mScenicDetails.name());
            values.put(Downloads.Impl.COLUMN_COVER_IMAGE, mScenicDetails.coverImage());
            if (mScenicDetails.offlinePackage != null) {
                values.put(Downloads.Impl.COLUMN_TOTAL_BYTES, mScenicDetails.offlinePackage.size);
            }
    ...
    public DownloadManager(Context context) {
        // this(context.getContentResolver(), context.getPackageName());
        mContext = context;
     ...
     */
    private int pauseDownload(long... ids) {
    ...
    public void pauseOrResumeDownload(long id) {
        boolean pause = false;
     ...
     */
    private int resumeDownload(long... ids) {
        if (ids == null || ids.length == 0) {
            throw new IllegalArgumentException("input param 'ids' can't be null");
        }

        // wake up download service
     ...
     * @throws FileNotFoundException if the destination file does not already
     *             exist
     */
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                if (DownloadManager.STATUS_SUCCESSFUL == status) {
                    ...
                    // TODO: if we ever add API to DownloadManager to let the
                    // caller specify
                    // non-external storage for a downloaded file, then the
                    // following code
    ...
    public void updateDownload(LongSparseArray<String> packages) {
        ContentValues values = new ContentValues();
//        for (int i = 0; i < packages.size(); i++) {
//            String fileName = Uri.parse(packages.valueAt(i)).getLastPathSegment();
//            Uri destUri = Uri.fromFile(new File(FileUtils.getExternalRootDir(),
//                    fileName));
//            values.put(Downloads.Impl.COLUMN_URI, Uri.parse(packages.valueAt(i)).toString());
//            values.put(Downloads.Impl.COLUMN_DESTINATION, Downloads.Impl.DESTINATION_FILE_URI);
//            values.put(Downloads.Impl.COLUMN_FILE_NAME_HINT, destUri.toString());
//            values.put(Downloads.Impl.COLUMN_CURRENT_BYTES, 0);
//            values.put(Downloads.Impl.COLUMN_TOTAL_BYTES, -1);
//            values.putNull(Downloads.Impl._DATA);
//            values.put(Downloads.Impl.COLUMN_STATUS, Downloads.Impl.STATUS_PENDING);
//            values.put(Downloads.Impl.COLUMN_FAILED_CONNECTIONS, 0);
//            long[] ids = new long[]{ packages.keyAt(i)};
//            mResolver.update(mBaseUri, values, getWhereClauseForIds(ids), getWhereArgsForIds(ids));
//        }
    }
     ...
     * 
     * @param context the {@link Context} to use for accessing the
     *            {@link ContentResolver}
     * @return maximum size, in bytes, of downloads that may go over a mobile
     *         connection; or null if there's no limit
     */
     ...
     * @param context the {@link Context} to use for accessing the
     *            {@link ContentResolver}
     * @return recommended maximum size, in bytes, of downloads that may go over
    ...
    /**
     * Adds a file to the downloads database system, so it could appear in
     * Downloads App (and thus become eligible for management by the Downloads
     * App).
     * <p>
     * It is helpful to make the file scannable by MediaScanner by setting the
     * param isMediaScannerScannable to true. It makes the file visible in media
     * managing applications such as Gallery App, which could be a useful
     * purpose of using this API.
     * 
     * @param title the title that would appear for this file in Downloads App.
     * @param description the description that would appear for this file in
     *            Downloads App.
     * @param isMediaScannerScannable true if the file is to be scanned by
     *            MediaScanner. Files scanned by MediaScanner appear in the
     *            applications used to view media (for example, Gallery app).
     ...
     * @param showNotification true if a notification is to be sent, false
     *            otherwise
     * @return an ID for the download entry added to the downloads app, unique
     *         across the system This ID is used to make future calls related to
     *         this download.
     */
