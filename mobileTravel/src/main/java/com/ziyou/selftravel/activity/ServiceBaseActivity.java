...
public class ServiceBaseActivity extends BaseActivity {
        ...
        public void onValidClick(View v) {
            ...
            switch (v.getId()) {
                case R.id.service_food: {
                    startServiceListActivity(ScenicShops.Type.FOOD,
                            getResources().getString(R.string.service_food_title),
                            getResources().getString(R.string.service_food_keyword), Const.KIND_FOOD);
                    break;
                }
                case R.id.service_hotel: {
                    startServiceListActivity(ScenicShops.Type.HOTEL,
                            getResources().getString(R.string.service_hotel),
                            getResources().getString(R.string.service_hotel_keyword), Const.KIND_HOTEL);
                    break;
                }
                case R.id.service_specialty: {
                    startServiceListActivity(ScenicShops.Type.SHOPPING,
                            getResources().getString(R.string.service_specialty_title),
                            getResources().getString(
                                    R.string.service_specialty_keyword), Const.KIND_SHOPPING);
                    break;
                }
                case R.id.service_wc: {
                    startServiceMapActivity(ScenicShops.Type.REST_ROOM,
                            getResources().getString(R.string.service_wc_title),
                            getResources().getString(R.string.service_wc_keyword), Const.KIND_WC);
                    break;
                }
                case R.id.service_traffic: {
                    if (mScenicDetailsModel.serviceInfo != null
                            && mScenicDetailsModel.serviceInfo.tranportInfo != null) {
                        Intent intent = new Intent(ServiceBaseActivity.this, TrafficActivity.class);
    ...
    protected void showServicePopup() {
        if (mServiceDialog == null) {
            View popupView = getLayoutInflater().inflate(
                    R.layout.scenic_details_service, null);
            GridView gridView = (GridView) popupView.findViewById(R.id.scenic_service_layout);
            gridView.setAdapter(new ScenicServiceAdapter(getApplicationContext(),
                    mServiceClickListener));

            mServiceDialog = new Dialog(ServiceBaseActivity.this, R.style.service_popup_dialog);
    ...
    private void showDialConfirmDialog(String dialogContent, final String phoneNumber) {
        if (mServiceConfirmDialog == null) {
            mServiceConfirmDialog = new CommonDialog(ServiceBaseActivity.this);
    ...
    private void startServiceListActivity(ScenicShops.Type type, String title, String keyword,
                                          String kind) {
        Intent intent = new Intent(ServiceBaseActivity.this, ServiceListActivity.class);
    ...
    private void startServiceMapActivity(ScenicShops.Type type, String title, String keyword,
                                         String kind) {
        Intent intent = new Intent(ServiceBaseActivity.this, ServiceMapActivity.class);
        intent.putExtra(Const.EXTRA_ACTION_BAR_TITLE, title);
        intent.putExtra(Const.EXTRA_SEARCH_KEYWORD, keyword);
        intent.putExtra(Const.EXTRA_SHOP_TYPE, type);
        intent.putExtra(Const.EXTRA_SEARCH_KIND, kind);
    ...
    private void postVideo() {
        Intent intent = new Intent(ServiceBaseActivity.this, VideoUploadActivity.class);
        intent.putExtra(Const.EXTRA_ID, mScenicDetailsModel.id);
        startActivity(intent);
    }

    private void postImages() {
        Intent intent = new Intent(ServiceBaseActivity.this, PostImagesActivity.class);
        intent.putExtra(Const.EXTRA_ID, mScenicDetailsModel.id);
        startActivity(intent);
    }
}
