...
public class StackBlurManager {
    static final float DEFAULT_LIMIT = 0.125f;
    ...
    public Bitmap process(int radius) {
        return process(radius, DEFAULT_LIMIT);
    }

    public Bitmap process(int radius, float limit) {
        _result = _blurProcess.blur(_image, radius, limit);
        return _result;
    }
    ...
    public void saveIntoFile(String path) {
        try {
            ...
            _result.compress(Bitmap.CompressFormat.PNG, 75, out);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
