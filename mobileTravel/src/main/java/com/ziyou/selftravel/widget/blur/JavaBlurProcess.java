...
package com.ziyou.selftravel.widget.blur;
...
/**
 * Blur using Java code. This is a compromise between Gaussian Blur and Box blur
 * It creates much better looking blurs than Box Blur, but is 7x faster than my
 * Gaussian Blur implementation. I called it Stack Blur because this describes
 * best how this filter works internally: it creates a kind of moving stack of
 * colors whilst scanning through the image. Thereby it just has to add one new
 * block of color to the right side of the stack and remove the leftmost color.
 * The remaining colors on the topmost layer of the stack are either added on or
 * reduced by one, depending on if they are on the right or on the left side of
 * the stack.
 * 
 * @author Enrique López Mañas <eenriquelopez@gmail.com> http://www.neo-tech.es
 *         Author of the original algorithm: Mario Klingemann
 *         <mario.quasimondo.com> Based heavily on
 *         http://vitiy.info/Code/stackblur.cpp See
...
class JavaBlurProcess implements BlurProcess {
    ...
    public Bitmap blur(Bitmap original, float radius, float limit) {
        if (limit <= 0 || limit > 1) {
            throw new IllegalArgumentException("limit should be (0, 1.0]");
        }
        int w = original.getWidth();
        int h = original.getHeight();
        int nw = (int) (w * limit);
        int nh = (int) (h * limit);
        int[] currentPixels = new int[nw * nh];
        int startX = (w - nw) >> 1;
        int startY = (h - nh) >> 1;
        original.getPixels(currentPixels, 0, nw, startX, startY, nw, nh);
        ...
        for (int i = 0; i < cores; i++) {
            horizontal.add(new BlurTask(currentPixels, nw, nh, (int) radius, cores, i, 1));
            vertical.add(new BlurTask(currentPixels, nw, nh, (int) radius, cores, i, 2));
        }
        ...
        return Bitmap.createBitmap(currentPixels, nw, nh, Bitmap.Config.ARGB_8888);
    }
    ...
    private static class BlurTask implements Callable<Void> {
        ...
        public BlurTask(int[] src, int w, int h, int radius, int totalCores, int coreIndex,
                int round) {
            _src = src;
            _w = w;
            _h = h;
