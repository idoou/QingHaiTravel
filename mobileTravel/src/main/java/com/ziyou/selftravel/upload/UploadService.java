...
public class UploadService extends IntentService {
    ...
    public static String NAMESPACE = "com.ziyou.selftravel";

    private static final String ACTION_UPLOAD_SUFFIX = ".ACTION_UPLOAD";
    ...
    /**
     * Utility method that creates the intent that starts the background file
     * upload service.
     * 
     * @param task object containing the upload request
     * @throws IllegalArgumentException if one or more arguments passed are
     *             invalid
     * @throws MalformedURLException if the server URL is not valid
     */
    public static void startUpload(final UploadRequest task) throws IllegalArgumentException,
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if (getActionUpload().equals(action)) {
                ...
                final ArrayList<FileToUpload> files = intent
                        .getParcelableArrayListExtra(PARAM_FILES);
                final ArrayList<NameValue> headers = intent
                        .getParcelableArrayListExtra(PARAM_REQUEST_HEADERS);
                final ArrayList<NameValue> parameters = intent
                        .getParcelableArrayListExtra(PARAM_REQUEST_PARAMETERS);
    ...
    private byte[] getTrailerBytes(final String boundary) throws UnsupportedEncodingException {
        ...
        builder.append(NEW_LINE).append(TWO_HYPHENS).append(boundary).append(TWO_HYPHENS)
                .append(NEW_LINE);
    ...
    private HttpURLConnection
            getMultipartHttpURLConnection(final String url, final String method,
    ...
    private void broadcastProgress(final String uploadId, final long uploadedBytes,
    ...
    private void broadcastCompleted(final String uploadId, final int responseCode,
    ...
    private void createNotification() {
        notification
                ...
                .setSmallIcon(notificationConfig.getIconResourceID()).setProgress(100, 0, true)
                .setOngoing(true);
    ...
    private void updateNotificationCompleted() {
        ...
        if (!notificationConfig.isAutoClearOnSuccess()) {
            notification.setContentTitle(notificationConfig.getTitle())
                    ...
                    .setSmallIcon(notificationConfig.getIconResourceID()).setProgress(0, 0, false)
                    .setOngoing(false);
