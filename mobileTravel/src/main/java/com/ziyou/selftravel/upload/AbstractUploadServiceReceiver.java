 ...
 * for {@link UploadService}. It provides the boilerplate code to properly
 * handle broadcast messages coming from the upload service and dispatch them to
 * the proper handler method.
 * 
...
public abstract class AbstractUploadServiceReceiver extends BroadcastReceiver {
            ...
            if (UploadService.getActionBroadcast().equals(intent.getAction())) {
                ...
                switch (status) {
                    ...
                    case UploadService.STATUS_COMPLETED:
                        final int responseCode = intent.getIntExtra(
                                UploadService.SERVER_RESPONSE_CODE, 0);
                        final String responseMsg = intent
                                .getStringExtra(UploadService.SERVER_RESPONSE_MESSAGE);
    ...
    /**
     * Register this upload receiver. It's recommended to register the receiver
     * in Activity's onResume method.
     * 
    ...
    /**
     * Unregister this upload receiver. It's recommended to unregister the
     * receiver in Activity's onPause method.
     * 
     ...
     * @param serverResponseMessage string containing the response received from
     *            the server
     */
